...
func (r *Resolver) PublicProcurementOrganizationUnitArticleInsertResolver(params graphql.ResolveParams) (interface{}, error) {
	var dataArray []structs.PublicProcurementOrganizationUnitArticle
	...
	err := json.Unmarshal(dataBytes, &dataArray)
	if err != nil {
		_ = r.Repo.CreateErrorLog(structs.ErrorLogs{Error: err.Error()})
		return errors.HandleAPPError(err)
	}
	...
	for _, data := range dataArray {
		...
		orgUnitID = data.OrganizationUnitID
		itemID := data.ID
		...
		if itemID != 0 {
			...
			_, err = r.Repo.UpdateProcurementOUArticle(itemID, &data)
			if err != nil {
				_ = r.Repo.CreateErrorLog(structs.ErrorLogs{Error: err.Error()})
				return errors.HandleAPPError(err)
			}
			...
			_, err := r.Repo.CreateProcurementOUArticle(&data)
			if err != nil {
				_ = r.Repo.CreateErrorLog(structs.ErrorLogs{Error: err.Error()})
				return errors.HandleAPPError(err)
			}
	...
	if notificationContent != "" {
		...
		employees, _ := GetEmployeesOfOrganizationUnit(r.Repo, orgUnitID)
		/*if err != nil {
			_ = r.Repo.CreateErrorLog(structs.ErrorLogs{Error: err.Error()})
			return errors.HandleAPPError(err)
		}*/
			...
			for _, user := range targetUsers {
				if user.ID == employee.UserAccountID && loggedInUser.ID != user.ID {
					...
					_, err := r.NotificationsService.CreateNotification(&structs.Notifications{
						...
						IsRead:      false,
						Data:        dataJSON,
						Path:        fmt.Sprintf("/procurements/plans/%d", planID),
					})
					if err != nil {
						_ = r.Repo.CreateErrorLog(structs.ErrorLogs{Error: err.Error()})
						return errors.HandleAPPError(err)
...
func (r *Resolver) PublicProcurementSendPlanOnRevisionResolver(params graphql.ResolveParams) (interface{}, error) {
	...
	for _, targetUser := range targetUsers {
		if loggedInUser.ID != targetUser.ID {
			...
			var content string
			if isRejected {
				content = "Zahtjev OJ " + organizationUnit.Title + " sa novim izmjenama je ažuriran i proslijeđen."
			} else {
				content = "Zahtjev OJ " + organizationUnit.Title + " je proslijeđen."
			}
			_, err := r.NotificationsService.CreateNotification(&structs.Notifications{
				Content:     content,
				Module:      "Javne nabavke",
				...
				FromContent: fmt.Sprintf("Menadžer %s", organizationUnit.Title),
				IsRead:      false,
				Data:        dataJSON,
			})
			if err != nil {
