...
func (repo *MicroserviceRepository) CreateMovementArticle(input dto.MovementArticle) (*dto.MovementArticle, error) {
	...
	_, err := makeAPIRequest("POST", repo.Config.Microservices.Accounting.MOVEMENT_ARTICLES, input, res)
	if err != nil {
		return nil, err
	}
...
func (repo *MicroserviceRepository) GetMovementArticles(id int) ([]dto.MovementArticle, error) {
	...
	_, err := makeAPIRequest("GET", repo.Config.Microservices.Accounting.MOVEMENT_ARTICLES, input, res)
	if err != nil {
		return nil, err
	}
