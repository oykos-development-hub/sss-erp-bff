...
type MicroserviceRepositoryInterface interface {
	...
	GetUsersByPermission(title config.PermissionPath, operation config.PermissionOperations) ([]structs.UserAccounts, error)
	...
	CreateFilledFinancialBudget(ctx context.Context, input structs.FilledFinanceBudget) error
	...
	FillActualFinancialBudget(ctx context.Context, id int, actual decimal.Decimal, Type int, requestID int) (*structs.FilledFinanceBudget, error)
	...
	GetRevisionTipImplementationByID(id int) (*structs.RevisionTipImplementations, error)
